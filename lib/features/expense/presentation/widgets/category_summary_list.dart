...
class CategorySummaryList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      final rawCategories = categoryController.categories.isNotEmpty
          ? categoryController.categories
          : ExpenseCategoryController.defaultCategories;

      // Get active categories
      final List<ExpenseCategoryEntity> categories = rawCategories
          .where((c) => !c.isDeleted)
          .toList();

      if (spentByCategory.containsKey('exp_archived') &&
          (spentByCategory['exp_archived'] ?? 0) > 0) {
        categories.add(
      ...
      // In compact report mode, prioritize showing categories with actual spending
      final List<ExpenseCategoryEntity> displayCategories;
      if (isCompact) {
        final withSpending = categories
            .where((c) => (spentByCategory[c.id] ?? 0.0) > 0)
            .toList();
        displayCategories =
            withSpending.isNotEmpty ? withSpending : categories;
      } else {
